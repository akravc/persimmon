...
class PersimmonParser extends RegexParsers with PackratParsers {
  ...
  lazy val pProgram: PackratParser[DefinitionLinkage] =
    (rep(pMixDef(Prog) | pFamDef(Prog))) ^^ { case famLists =>
      val fams = famLists.flatten
