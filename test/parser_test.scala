...
class ParserTesting extends AnyFunSuite {
  ...
  test("parse - ex: abcode_dot") {
    val p = readFile("res/abcode_dot")
    assert(canParse(pProgram, p))
  }
  test("parse - ex: abcodepaper_dot") {
  ...
  test("parse - famdef one type") {
    ...
    assertResult(
      ("A", FamsDefn("A", List("A" -> DefinitionLinkage(
  ...
  test("parse - famdef extends") {
    ...
    assertResult(("A", FamsDefn("A", List(map)))){parseSuccess(pFamDef(Prog), fam)}
  }
